...
//    exit;
        ...
        if(!$this->mOtp->CheckUserExists($uid)){
            ...
            if(OCP\Config::getAppValue('user_otp','authMethod',_AUTH_DEFAULT_)===_AUTH_OTP_){
                $result = $this->mOtp->CheckToken($password);
            }else{
                $result = $this->mOtp->CheckToken($_POST['otpPassword']);            
            }
            if ($result===0){
                if(OCP\Config::getAppValue('user_otp','authMethod',_AUTH_DEFAULT__)===_AUTH_OTP_){
                    return $uid;
                }else{
                    return parent::checkPassword($uid, $password);
                }
